    ...
    public class SimpleBotLogic : IBotLogic
    {
        ...
        private static PlayerAction ActOnFirstRound(Player bot, Table table)
        {
                ...
                Math.Abs((int) bot.PocketCards[0].Rank - (int) bot.PocketCards[1].Rank) is 1)
            {
                ...
                if (betAmount is 0)
                {
                    if (Random.Next(0, 100) < 20 && bot.StackMoney >= table.BigBlind)
                    {
            ...
            if (bot.HandValue > 40)
            {
                if (Random.Next(0, 100) < 50)
                {
                    if (betAmount > 0)
                        return new PlayerAction
                            {ActionType = PlayerActionType.Call, PlayerIndexNumber = bot.IndexNumber};
                    
                    var myBet = Random.Next(0, 100) < 50
                        ? table.BigBlind
                        : table.BigBlind * 2;
        ...
        private static PlayerAction ActOnSecondRound(Player bot, Table table)
        {
            ...
            if (bot.Hand is HandType.OnePair || bot.Hand is HandType.TwoPairs)
            {
                ...
                if (betAmount is 0)
                {
                    ...
                    {
                        var myBet = table.BigBlind * (1 + (float) Random.Next(0, 25) / 100);
                        if (myBet <= bot.StackMoney)
                            return new PlayerAction
                            {
            ...
            if (bot.HandValue >= 1020) // Triple of 2
            {
                if (betAmount is 0)
                {
                    var myBet = Random.Next(100, table.Pot.TotalAmount * 30) / 100;
        ...
        private static PlayerAction ActOnFourthRound(Player bot, Table table)
        {
            ...
            if (bot.Hand is HandType.TwoPairs)
            {
                ...
                {
                    var myBet = Random.Next(table.Pot.TotalAmount / 30, table.Pot.TotalAmount);
