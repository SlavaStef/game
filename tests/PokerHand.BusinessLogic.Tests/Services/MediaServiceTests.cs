...
namespace PokerHand.BusinessLogic.Tests.Services
{
    public class MediaServiceTests
    {
        ...
        [Fact]
        public async void HasCustomProfileImage_ReturnsTrue_IfPlayerImageIsNotEqualToDefaultImage()
        {
            // Arrange
            var mockFileSystem = new MockFileSystem();
            ...
            // Act
            var result = await sut.HasCustomProfileImage(playerId);
        ...
        [Fact]
        public async void HasCustomProfileImage_ReturnsFalse_IfPlayerImageIsEqualToDefaultImage()
        {
            // Arrange
            var mockFileSystem = new MockFileSystem();
            ...
            var path = Path.Combine(AppContext.BaseDirectory, "wwwroot", "profileImages", $"{playerId}");
            var file = new string(DefaultImage);
            var mockFile = new MockFileData(file);
            mockFileSystem.AddFile(path, mockFile);
